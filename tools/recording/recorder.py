...
class ProGuiRecorder:
    def __init__(self, output_dir=None, audio_device=None):
        ...
        self.record_indicator = tk.Label(
            self.mini_frame, text="●", fg="white", bg="#e74c3c", font=("Arial", 16)
        )
    ...
    def run_ffmpeg(self):
        if sys.platform == "darwin":
            ...
            cmd = ["ffmpeg", "-y", "-f", "gdigrab", "-framerate", "30", "-i", "desktop"]
            if self.audio_device:
        ...
        with open(self.log_file, "w", encoding="utf-8") as f:
            self.process = subprocess.Popen(
                cmd, stdin=subprocess.PIPE, stdout=f, stderr=subprocess.STDOUT, env=env
            )
            self.process.wait()
    ...
    def show_post_action_dialog(self):
        ...
        def do_create_draft():
            ...
            name = tk.simpledialog.askstring(
                "创建草稿", "请输入剪映项目名称:", initialvalue=default_name, parent=dialog
            )
            if not name:
                return
