...
def build_movie_commentary(
    video_path, storyboard_path, project_name="AI_Movie_Commentary", bgm_path=None, mask_path=None
):
    ...
    for i, scene in enumerate(storyboard):
        ...
        if len(parts) == 2:
            src_start_us = (parts[0] * 60 + parts[1]) * 1000000
        else:
            src_start_us = (parts[0] * 3600 + parts[1] * 60 + parts[2]) * 1000000

        duration_us = int(duration * 1000000)

        # A. 添加主视频片段
        project.add_media_safe(
            video_path, timeline_cursor, duration_us, "MainTrack", source_start=src_start_us
        )
        ...
        if text:
            ...
            if mask_path and os.path.exists(mask_path):
                project.add_media_safe(
                    mask_path, timeline_cursor, duration_us, "MaskTrack", transform_y=-0.85
                )
