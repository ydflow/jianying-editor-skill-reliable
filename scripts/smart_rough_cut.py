...
try:
    ...
    warnings.warn(
        f"[smart_rough_cut] Optional dependency missing: {e}. "
...
def analyze_video_content(video_path):
    print("[*] Analyzing video content using Gemini-3-Pro...")
    ...
    try:
        response = client.chat_completion(
            messages=[{"role": "user", "content": prompt}], model=model, file_paths=[video_path]
        )
